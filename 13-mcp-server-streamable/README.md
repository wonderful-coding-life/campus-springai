...
기존 도구(Tool)에서 사용하던 애노테이션과 ToolContext만 다음과 같이 바꾸면 된다.

- @Tool → @McpTool
- @ToolParam → @McpToolParam
- ToolContext toolContext → McpMeta mcpMeta
```java
...
MCP Inspector는 node.js 기반으로 MCP 서버 테스트를 위한 UI를 제공함. (https://nodejs.org/ko/download)
